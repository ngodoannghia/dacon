...
def train():
    ...
    path_train = "E://competition//atoms//data//train.csv"
    ...
    for train_index, test_index in kf.split(df_train):
        fold += 1
        X_train = df_train.values[train_index]
        X_val = df_train.values[test_index]
        
        # X_train.reset_index(inplace=True)
        # X_val.reset_index(inplace=True)
        
        X_train = pd.DataFrame(columns=df_train.columns, data=X_train)
        X_val = pd.DataFrame(columns=df_train.columns, data=X_val)
        ...
        for epoch in range(epochs):
            ...
            if max_score < score:
                torch.save(model.state_dict(), f"E://competition//atoms//models//best_epoch_{fold}.pth")
                max_score = score
        ...
        model.load_state_dict(torch.load(f'E://competition//atoms//models//best_epoch_{fold}.pth'))
        model.eval()
        predictions = []
