...
tokenizer = AutoTokenizer.from_pretrained("E://competition//atoms//ChemBERTa-zinc-base-v1")
model_chem_bert = AutoModelForMaskedLM.from_pretrained("E://competition//atoms//ChemBERTa-zinc-base-v1")
...
def create_feature(sample):
    ...
    fingerprint = Chem.RDKFingerprint(mol, fpSize=32).ToList()
    ...
    return [inertial_shape_factor, fraction_csp3, qed_value, balaban_j, mol_weight, logp, tpsa, mol_refractivity, chi0, pmi1, pmi2, pmi3, pmi_ratio, radius_of_gyration, num_aromatic_rings, num_h_donors, num_h_acceptors, num_rings, num_rotatable_bonds] + fingerprint
...
def create_feature_chembert(sample):
    input_ids = torch.tensor([tokenizer.encode(sample)])

    with torch.no_grad():
        features = model_chem_bert(input_ids)
    
    return torch.mean(features[0], 1).reshape(-1).tolist()

def read_data_train(path):
    train = pd.read_csv(path)
    data_train = pd.concat([train['Molecule ChEMBL ID'], train['Smiles'], train['IC50_nM'], train['pIC50']], axis=1)
...
def create_data_train(data_train):
    ...
    for i in range(len(data_train)): 
        ...
        smile, lb = row[1], row[3]
        
        feat_1 = create_feature(smile)
...
if __name__ == '__main__':
    ...
    data_test = read_data_test(path_test)
    ...
    features, labels = create_data_train(data_train)
